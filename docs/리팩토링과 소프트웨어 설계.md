...
이전 프로젝트에서 설계에 문제가 있는 코드가 있어서 고쳐보려고 합니다. 만약 이런 코드를 계속 방치하게 된다면 아래와 같은 문제점이 발생합니다.

- 새로 온 합류한 팀원이 프로젝트를 이해하는 데 시간이 오래 걸림
  - 새로 온 팀원의 업무 시작이 늦어짐
- 프로젝트 관리가 어려워진다.
- 다른 곳에도 영향을 준다.

이전 프로젝트의 스케줄러, 에러 컨트롤러에서 아래의 문제점을 가지고 있었습니다.
...
이렇게 리팩토링하면 이해하기 쉬운 코드가 됩니다. 이런 코드는 새로운 개발자가 팀에 합류할 때 프로젝트를 파악하는 시간이 짧아지고 업무에 빠르게 적응할 수 있습니다.

그러면 스케줄러와 에러 컨트롤러를 어떻게 개선했는지 설명하겠습니다.
...
기준 거리 구간은 다음과 같습니다. [5, 10, 15, 20, 25, 30, 40, 50, 60, 60초과]

주문의 출발지와 도착지간 거리가 이 기준 중 하나보다 작거나 같고, 그보다 더 작은 기준 거리보다 크다면 해당 기준 거리 구간에 포함됩니다.
(예를 들어, 11~15km 거리는 15km, 정확히 10km는 10km 기준 구간으로 분류됩니다.)
```
...
- 주문 정보를 기준 거리 구간에 맞게 분류 후 주문 금액을 합산한다.
- 기준 거리 구간마다 저장한 주문 정보 개수로 주문 금액의 평균을 계산한다.
...
```txt
서버 개발자가 에러 컨트롤러를 만들었습니다. 하지만 클라이언트 개발자가 서버의 에러 응답을 분간하기 어려워 개선을 요구합니다.
...
이 계층구조를 활용해 클라이언트에게 아래와 같이 일관된 형식의 에러 응답을 해야 합니다.
...
기존에는 서버에서 에러가 발생하면 에러 컨트롤러가 처리합니다.
에러 컨트롤러는 상태코드가 404이면 클라이언트에게 응답합니다.
상태코드가 404가 아니라면 Slack API과 로거를 통해 개발자에게 에러를 알리고 클라이언트에게 응답합니다.

또한 사용자의 요청에서 문제가 발생한 게 아니라 서버 내부의 문제라면 로거로 기록하고 Slack API를 통해 개발자에게 메시지를 전송해야 합니다.
...
리팩토링 후 코드는 이 글에 넣기에 많아서 [Github 링크](https://github.com/daniel-juyeon-kim/quicker-back-v2/tree/main/controllers/error)와 간략하게 도식화한 그림으로 대체합니다.
...
classDiagram
    ...
    class ErrorController {
        <<interface>>
        +handleError(error: ErrorTypes, req: Request, res: Response, next: NextFunction)
    }
    ...
    class DataBaseErrorController {
        +handle(error: ErrorTypes, res: Response, date: Date)
        -handleBusinessRuleConflictDataError(error: DataBaseError, res: Response)
        -handleDuplicateDataError(error: DataBaseError, res: Response)
        -handleNotExistDataError(error: DataBaseError, res: Response)
    }

    class ExternalApiErrorController {
    ...
    class UnknownErrorController {
        -logger: ErrorLogger
        -messageBot: ErrorMessageBot
        +handle(error: ErrorTypes, res: Response, date: Date)
        -sendSlackMessage(error: ErrorTypes, date: Date)
    }

    class CommonExternalApiErrorHandler {
        <<abstract>>
        -logger: ErrorLogger
        -errorMessageBot: ErrorMessageBot
        +sendErrorMessageBySlack(error: ExternalApiError, date: Date)
    }

    class ErrorMessageBotErrorHandler {
        +handle(error: ExternalApiError, date: Date)
    }

    class SmsApiErrorHandler {
        +handle(error: ExternalApiError, date: Date)
        -sendErrorMessageBySlack(error: ExternalApiError, date: Date)
    }

    class TmapApiErrorHandler {
        +handle(error: ExternalApiError, date: Date)
        -sendErrorMessageBySlack(error: ExternalApiError, date: Date)
    }
    ...
    ErrorControllerImpl --> DataBaseErrorController
    ErrorControllerImpl --> ExternalApiErrorController
    ErrorControllerImpl --> UnknownErrorController

    ExternalApiErrorController --> ErrorMessageBotErrorHandler
    ExternalApiErrorController --> SmsApiErrorHandler
    ExternalApiErrorController --> TmapApiErrorHandler

    CommonExternalApiErrorHandler <|-- SmsApiErrorHandler
    CommonExternalApiErrorHandler <|-- TmapApiErrorHandler

    ExternalApiErrorHandler <|.. CommonExternalApiErrorHandler
```
