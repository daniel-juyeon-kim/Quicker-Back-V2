...
describe("validateSingleImageFile", () => {
  ...
  test("실패하는 테스트, 지원하지 않는 확장자", () => {
    ...
    expect(res.send).toHaveBeenCalledWith(new HttpErrorResponse(400, "허용하지 않는 확장자 입니다."));
  });
  ...
  test("통과하는 테스트, 파일이 존재하지 않음", () => {
    ...
    expect(next).not.toHaveBeenCalled();
    expect(res.send).toHaveBeenCalledWith(new HttpErrorResponse(400, "이미지 파일이 존재하지 않습니다."));
  });
});
